...
/** Verified or user-confirmed records only — pending/rejected never count. */
async function buildEvidenceCatalogue(
  ...
  const { data: experiences } = await ctx.db
    .from('experiences')
    .select('id, company, title, start_date, end_date, is_current, verification_status')
  ...
  const { data: education } = await ctx.db
    .from('education')
    .select('id, institution, qualification, field_of_study, verification_status')
    .in('verification_status', verified)
